...
@hook.command('grab', autohelp=False)
def grab(text, nick, chan, db, notice):
    """grab <nick> grabs the last message from the specified user as a quote"""
    if text.lower() == nick.lower():
        return "Don't be the next /u/aalewis and quote yourself. smh"

    for item in conn.history[chan].__reversed__():
        name, timestamp, msg = item
        if text.lower() == name.lower():
            # Found the most recent quote, add it
            qadd(text, nick, chan, db, notice)
            return
    notice("I couldn't find anything from {} in recent history.".format(text))
